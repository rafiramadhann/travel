...
class hotel_model
{
    ...
    {
        $query = "SELECT {$this->table}.`name`, {$this->table}.`slug`, {$this->table}.`daerah_id`, {$this->table}.`price`, {$this->table}.`facility`, {$this->table}.`image` FROM `{$this->table}` LEFT JOIN `daerah` ON `{$this->table}`.`daerah_id` = `daerah`.`daerah_id` WHERE `daerah`.`slug` = '$slug'";
        $this->db->query($query);
        return $this->db->resultSet();
    }
    ...
    public function add()
    {
        ...
        $facility = $this->db->filter($_POST['facility']);
